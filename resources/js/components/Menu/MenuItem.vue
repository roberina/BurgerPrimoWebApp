<template>
  ...
  >
    <div class="flex-1 p-4 flex flex-col justify-between min-w-0">
      <div>
        <div class="flex items-center gap-2 mb-1.5">
          ...
          <span v-if="item.discount_percentage" class="text-xs bg-[#D2691E]/15 text-orange-400 px-2 py-0.5 rounded-full font-semibold">-{{ item.discount_percentage }}%</span>
        </div>
        <h3 class="font-bold text-sm leading-snug text-white">{{ ln(item.name, item.name_en) }}</h3>
        <p class="text-xs mt-1 line-clamp-2 leading-relaxed text-gray-400">{{ ln(item.description, item.description_en) }}</p>
      </div>
      <div class="flex items-center gap-2 mt-3">
        <span class="font-bold text-sm text-white">{{ Number(item.price).toFixed(2) }}€</span>
        <span v-if="item.original_price && Number(item.original_price) > Number(item.price)" class="text-xs line-through text-gray-400">{{ Number(item.original_price).toFixed(2) }}€</span>
      </div>
    </div>
    <div class="relative flex-shrink-0 w-28">
      ...
      <button @click.stop="openAddonModal" :aria-label="t('addon.add')" class="group/btn absolute bottom-2 right-2 w-8 h-8 bg-[#D2691E] hover:bg-[#E07A2E] text-white rounded-lg flex items-center justify-center shadow-lg transition-all duration-200 opacity-0 group-hover:opacity-100 translate-y-1 group-hover:translate-y-0 cursor-pointer">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 absolute transition-all duration-150 group-hover/btn:opacity-0 group-hover/btn:scale-50" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4" /></svg>
        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 absolute opacity-0 scale-50 transition-all duration-150 group-hover/btn:opacity-100 group-hover/btn:scale-100" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" /></svg>
      </button>
    </div>
  ...
  <div v-else class="rounded-2xl overflow-hidden hover:ring-2 hover:ring-[#D2691E] transition-all duration-300 group cursor-pointer bg-[#121212]" @click="openDetailModal">
    <div class="relative aspect-[4/3] overflow-hidden bg-[#0B0B0B]">
      ...
      <button @click.stop="toggleFavorite" :aria-label="item.is_favorited ? t('menu.item.fav.remove') : t('menu.item.fav.add')" class="absolute top-3 right-3 w-10 h-10 bg-black/50 hover:bg-black/70 rounded-lg flex items-center justify-center transition-all duration-200 z-10 cursor-pointer">
        <svg xmlns="http://www.w3.org/2000/svg" :class="['h-6 w-6 transition-all', item.is_favorited ? 'fill-red-500 text-red-500' : 'fill-none text-white']" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" /></svg>
      </button>
      <button @click.stop="openAddonModal" :aria-label="t('addon.add')" class="group/btn absolute bottom-3 right-3 w-12 h-12 bg-[#D2691E] hover:bg-[#E07A2E] text-white rounded-xl flex items-center justify-center transition-all duration-200 shadow-lg hover:shadow-[#D2691E]/50 hover:scale-110 cursor-pointer">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 absolute transition-all duration-150 group-hover/btn:opacity-0 group-hover/btn:scale-50" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4" /></svg>
        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 absolute opacity-0 scale-50 transition-all duration-150 group-hover/btn:opacity-100 group-hover/btn:scale-100" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" /></svg>
      </button>
    </div>
    <div class="p-4">
      <h3 class="text-lg font-bold mb-2 text-white">{{ ln(item.name, item.name_en) }}</h3>
      <p class="text-sm mb-4 line-clamp-2 min-h-[40px] text-gray-400">{{ ln(item.description, item.description_en) }}</p>
...
interface MenuItemData {
  id: number; name: string; name_en?: string | null; description: string; description_en?: string | null; price: number;
